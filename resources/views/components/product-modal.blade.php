...
@if ($productoDetalle && $detalleProducto)
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4" x-data="{ tab: 'info' }"
        wire:key="modal-{{ $productoDetalle }}">

        {{-- Backdrop --}}
        <div class="absolute inset-0 bg-black/60 backdrop-blur-sm cursor-pointer" wire:click="cerrarDetalle"></div>

        {{-- Panel --}}
        <div
            class="relative z-10 bg-white rounded-3xl shadow-2xl w-full max-w-4xl
            ...
            <div class="grid md:grid-cols-2 gap-0">
                ...
                <div class="relative bg-gray-50 rounded-t-3xl md:rounded-l-3xl md:rounded-tr-none overflow-hidden"
                    ...
                    <div class="absolute top-4 left-4 flex gap-2">
                        <span
                            class="{{ $detalleProducto->tipo === 'servicio' ? 'bg-blue-500' : 'bg-[#E48F62]' }}
                ...
                <div class="p-7 flex flex-col gap-4">
                    ...
                    @if ($detalleProducto->tipo === 'producto')
                        ...
                        <div class="flex items-center gap-2">
                            <span
                                class="w-2 h-2 rounded-full {{ $disp > 0 ? 'bg-green-400' : 'bg-red-400' }} inline-block"></span>
                                ...
                                <div class="bg-[#fafaf8] rounded-xl p-3 border border-[#f0ede8]">
                                    <div class="flex items-center justify-between mb-1">
                                        ...
                                        <div class="flex gap-0.5">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <i
                                                    class="fa-solid fa-star text-[10px]
                    ...
                    <div class="flex flex-col gap-3 mt-auto pt-2">
                        ...
                        {{-- Fila 1: Añadir al carrito / Agregar servicio + Favoritos --}}
                       ...
                       hover:scale-[1.02] active:scale-[0.98] hover:shadow-md">
                            ...
                            @else
                                <button onclick="agregarAlCarrito({{ $detalleProducto->Id }}, this)"
                                    class="flex-1 bg-white border-2 border-blue-500 text-blue-500 font-bold
                       py-3 rounded-2xl transition-all duration-200 flex items-center justify-center gap-2
                       ...
                       hover:scale-[1.02] active:scale-[0.98] hover:shadow-md">
                            ...
                            {{-- Favoritos --}}
                            <button data-deseo-btn="{{ $detalleProducto->Id }}"
                   ...
                   hover:scale-110 active:scale-95"
                        ...
                        {{-- Fila 2: Comprar / Contratar ahora — aplica a productos Y servicios --}}
                        @auth('usuario')
                            <a href="{{ route('checkout.comprar-ahora', $detalleProducto->Id) }}"
                                class="w-full flex items-center justify-center gap-2 text-white font-bold
                   py-3 rounded-2xl transition-all duration-200
                   hover:scale-[1.02] active:scale-[0.98] hover:shadow-md
                   {{ $detalleProducto->tipo === 'producto' ? 'bg-[#E48F62] hover:bg-[#d07a50]' : 'bg-blue-500 hover:bg-blue-600' }}
                   {{ $detalleProducto->cantidad_disponible < 1 ? 'opacity-50 pointer-events-none' : '' }}">
                                <i
                                    class="fa-solid {{ $detalleProducto->tipo === 'producto' ? 'fa-bolt' : 'fa-calendar-check' }}"></i>
                                {{ $detalleProducto->tipo === 'producto' ? 'Comprar ahora' : 'Contratar ahora' }}
                            </a>
                        @else
